...
def create_map(t_lat=0, t_lon=0):
    ...
    html_java = Element(htmp_script)
    map.get_root().html.add_child(html)
    map.get_root().html.add_child(html_java)
...
#Opens and renders the template for home.html
@app.route('/')
def initial(): 
...
@app.route('/search_data', methods=['POST'])
def direction_data():
    global user_lat, user_lon, dir_lon, dir_lat, dir_name, html, mode, direction_time_text
    ...
    direction_time_text = data['routes'][0]['legs'][0]['duration']['text']
    waypoints = [] 
    warnings = []
    ...
    if mode == 'transit':
        create_direction(road_snapped_waypoints)
    else:
...
def calculate():
    ...
    error_minutes = 8
    direction_time_min = direction_time // 60 
    total_min = direction_time_min + user_time + error_minutes
    ...
    return render_template('result.html', hours = time_hours, minutes = time_minutes, status = AM_PM)    
